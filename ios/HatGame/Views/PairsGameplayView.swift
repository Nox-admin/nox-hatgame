...
/// Экран игрового процесса попарного режима
struct PairsGameplayView: View {
    ...
    init(viewModel: PairsGameViewModel) {
    ...
    // MARK: - Game Controls (extracted to fix Swift type-checker complexity — BUG-032)

    @ViewBuilder
    private var gameControls: some View {
        if viewModel.phase == .playing && !viewModel.isPaused {
            timerSection
                .offset(x: timerShakeOffset)
                .padding(.bottom, 16)
            // BUG-041: "Завершить ход" отдельно выше основных кнопок, с явным разрывом
